 ...
 *   - pts_trained_stable_v{samplesVersion}_regiao_4_trained
 *   - pts_trained_stable8910_v{samplesVersion}_regiao_4_trained
 *   - Hand-drawn reinforcement polygons (imports block)
 *
 * OUTPUTS
 *   - PANT_col11_reg4_v{outputVersion} (one band per year, 1985-2025)
 *
 * NOTES
 *   - Set flagCollectionSamples to true to preview a few years on the map
 *     without queueing an export.
 *   - A second variant of this region without the manual polygons was kept in the
 *     original folder; it is superseded by this one.
 *
 * PIPELINE
...
var manualSamplesLimit = 350; // Pixels drawn at random inside the polygons, per year.
...
var bandasOnTheFly = [
  ...
  "gemi_median", "gemi_median_wet", "awei_median_dry", "awei_median", "awei_median_wet", "evi2_median",  "gv_median", "gemi_median_dry" // Extras from Reg 4 Col 10
];
...
var bandasAsset = [
  ...
  'swir1_median', 'nir_median', 'red_median', 'blue_median', 'green_median', 'swir2_median' // Extras from Reg 4 Col 10
];
...
// Define explicit model classification features (Region 4 band list mapped from Collection 10)
var bandNamesList = ee.List([
   "longitude", "latitude", "brightness_median", "avi_median", "green_median_dry",
   "iia_median", "wetness_median", "red_median_wet", "avi_median_wet", "brba_median",
   "ratio_median", "lai_median", "ndvi_median_wet", "brightness_median_dry", "merit_dem",
   "savi_median_wet", "brba_median_wet", "evi_median_wet", "nir_median_dry", "wetness_median_dry",
   "nir_median_wet", "green_median_wet", "rvi_median", "avi_median_dry", "ri_median", "roughness",
   "osavi_median_wet", "hallheigth_median_dry", "iia_median_wet", "nddi_median", "wetness_median_wet",
   "brightness_median_wet", "ri_median_wet", "swir1_median_dry", "ratio_median_wet", "swir2_median_dry",
   "ndvi_median_dry", "swir2_median_wet", "mndwi_median_wet", "evi_median", "hallcover_median_dry",
   "brba_median_dry", "rvi_median_wet", "osavi_median", "mndwi_median", "red_median_dry", "gcvi_median",
   "gcvi_stdDev", "gcvi_median_wet", "dswi5_median"
...
// Define maximum sample allocations for stable training databases (Region 4 values)
var samplesForestLimit       = 1000;
var samplesSavannaLimit      = 2000;
var samplesGrasslandLimit    = 200;
var samplesAgricultureLimit  = 500;  // default base; overridden dynamically per year below
var samplesWaterLimit        = 900;
var samplesNonVegetatedLimit = 200;

// Define sample thresholds for complementary datasets (Region 4 values)
var compCaveLimit     = 1;
...
for (var yearIndex = 0; yearIndex < historicalYearsArray.length; yearIndex++) {
    ...
    // Dynamic adjustment of agriculture training allocation parameters (Region 4 progression preserved)
    ...
    else if (operationalYear >= 2021 && operationalYear <= 2022) { samplesAgricultureLimit = 2900; }
    ...
    // NOTE: agric (ref 19) is intentionally commented out/excluded for Region 4 complementary merge, identical to Col 10 script behavior
 ...
 // ------------------------------------------------------------------------------
    ...
    if (flagCollectionSamples) {
      Map.addLayer(imgv1.select('classification_' + operationalYear), visualizationParams, 'RF Classified v1 Output ' + operationalYear, false);
        ...
        Map.addLayer(annuallyClassifiedImage, visualizationParams, 'RF Classified v2 Output ' + operationalYear, false);

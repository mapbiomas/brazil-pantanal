/**
 * ==============================================================================
 * p01 | Regional classification - region 3
 * ------------------------------------------------------------------------------
 * SUMMARY
 *   Annual land use and land cover classification for Region 3 - Barao de Melgaco.
 *   
 ...
 *   - pts_trained_stable_v{samplesVersion}_regiao_3_trained
 *   - pts_trained_stable8910_v{samplesVersion}_regiao_3_trained
 *
 * OUTPUTS
 *   - PANT_col11_reg3_v{outputVersion} (one band per year, 1985-2025)
 *
 * NOTES
 ...
 *
 * PIPELINE
 *   p02 classification | region 3 of 0-8  ->  p02_merge_regions_gapfill
 * ==============================================================================
 */
...
var targetedYearsList = [1989,2024];
...
// Define explicit model classification features (Region 3 band list)
var bandNamesList = ee.List([
     "brightness_median", "longitude", "wetness_median", "savi_median_wet",
     "swir1_median_dry", "avi_median", "brightness_median_dry", "wetness_median_dry",
     "nir_median_wet", "merit_dem", "swir2_median_dry", "avi_median_wet",
     "nir_median_dry", "ratio_median", "red_median_dry", "mndwi_median_wet",
     "brightness_median_wet", "hallheigth_median_dry", "wetness_median_wet", "gcvi_median_wet",
     "gcvi_stdDev", "green_median_wet", "avi_median_dry", "evi_median_wet",
     "ri_median", "ri_median_wet", "shade_median", "osavi_median_wet", "savi_median",
     "iia_median_wet", "hallcover_median_dry", "green_median_dry", "shade_median_dry",
     "evi_median_dry", "iia_median", "rvi_median", "lai_median", "swir1_median_wet",
     "ratio_median_dry", "gvmi_median", "gvmi_median_wet", "shade_median_wet", "gcvi_median",
     "swir2_median_wet", "dswi5_median", "red_median_wet", "ratio_median_wet",
     "ui_median", "dswi5_median_dry", "bsi_median"
 ]);
...
// Define maximum sample allocations for stable training databases (Region 3 values)
var samplesForestLimit       = 600;
...
// Define sample thresholds for complementary datasets (Region 3 values)
var compCaveLimit     = 1;
var compSavannaLimit  = 2000;
var compGrasslandLimit = 50;
var compForestLimit   = 50;
var compAgroLimit     = 250;
var compWaterLimit    = 50;
...
for (var yearIndex = 0; yearIndex < historicalYearsArray.length; yearIndex++) {
    ...
    // Dynamic adjustment of training allocation parameters (Region 3 progression)
 // Dynamic adjustment of training allocation parameters (Region 2 progression)
    if (operationalYear >= 1990) { samplesAgricultureLimit = 1200; }
    if (operationalYear >= 2000) { compStrictAgricLimit = 1500; }
    if (operationalYear >= 2010) { compStrictAgricLimit = 2000; }
    ...
    // Irrigated agriculture (19) is rare early in the series and grows later. (Region 3 values)
    ...
    // NOTE: Region 3 does NOT include agric (ref 19) in the complementary merge (preserved from Col 10)
    ...
    // NOTE: agric (ref 19) excluded from base merge for Region 3, consistent with Col 10 behavior

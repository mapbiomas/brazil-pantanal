 ...
 *   - pts_trained_stable_v{samplesVersion}_regiao_6_trained
 *   - pts_trained_stable8910_v{samplesVersion}_regiao_6_trained
 *
 * OUTPUTS
 *   - PANT_col11_reg6_v{outputVersion} (one band per year, 1985-2025)
 *
 * NOTES
 *   - Set flagCollectionSamples to true to preview a few years on the map
 *     without queueing an export.
 *   - This region adds the GEMI and AWEI indices and three extra mosaic bands to
 *     the predictor stack.
 *
 * PIPELINE
...
var bandasOnTheFly = [
  ...
  "gemi_median", "gemi_median_wet", "gemi_median_dry", "awei_median", "awei_median_wet", "awei_median_dry" // Extras Reg 6
];
...
var bandasAsset = [
  ...
  'swir1_median', 'nir_median', 'red_median', 'blue_median', 'green_median', 'swir2_median',  // Extras Reg 6
];
...
// Define explicit model classification features (Region 6 band list mapped exactly from Collection 10)
var bandNamesList = ee.List([
  ...
  "avi_median_wet", "nir_median_dry", "brba_median", "wetness_median_wet", "nir_median_wet",
  "evi_median", "swir1_median_dry", "mndwi_median", "wetness_median_dry", "green_median_dry",
  "osavi_median", "brightness_median_wet", "savi_median", "evi_median_wet", "green_median_wet",
  "avi_median_dry", "gcvi_stdDev", "hallheigth_median_dry", "ri_median", "mndwi_median_wet",
  "gcvi_median_wet", "iia_median_wet", "ratio_median", "iia_median", "rvi_median",
  "swir1_median_wet", "swir2_median_dry", "mndwi_median_dry", "red_median_dry", "evi_median_dry",
  "red_median_wet", "rvi_median_dry", "osavi_median_wet", "ratio_median_wet", "lai_median",
  "rvi_median_wet", "ndvi_median_wet", "hallcover_median_dry", "co2flux_median"
...
// Define maximum sample allocations for stable training databases (Region 6 values)
var samplesForestLimit       = 550;
var samplesSavannaLimit      = 4000;
var samplesGrasslandLimit    = 1000;
var samplesAgricultureLimit  = 600;  // default base; overridden dynamically per year below
var samplesWaterLimit        = 900;
var samplesStrictAgricLimit  = 50;  // Ref 19 specific for Reg 6

// Define sample thresholds for complementary datasets (Region 6 values)
var compCaveLimit     = 1;
var compSavannaLimit  = 3000;
...
for (var yearIndex = 0; yearIndex < historicalYearsArray.length; yearIndex++) {
    ...
    // Dynamic adjustment of agriculture training allocation parameters (Region 6 progression preserved)
    if (operationalYear <= 2000) { samplesAgricultureLimit = 300; }
    else if (operationalYear == 2001) { samplesAgricultureLimit = 300; }
    else if (operationalYear == 2002) { samplesAgricultureLimit = 400; }
    else if (operationalYear == 2003) { samplesAgricultureLimit = 400; }
    else if (operationalYear == 2004) { samplesAgricultureLimit = 450; }
    else if (operationalYear == 2005) { samplesAgricultureLimit = 500; }
    else if (operationalYear == 2006) { samplesAgricultureLimit = 550; }
    else if (operationalYear == 2007) { samplesAgricultureLimit = 550; }
    else if (operationalYear == 2008) { samplesAgricultureLimit = 550; }
    else if (operationalYear == 2009) { samplesAgricultureLimit = 600; }
    else if (operationalYear == 2010) { samplesAgricultureLimit = 600; }
    else if (operationalYear == 2011) { samplesAgricultureLimit = 600; }
    else if (operationalYear == 2012) { samplesAgricultureLimit = 750; }
    else if (operationalYear == 2013) { samplesAgricultureLimit = 750; }
    else if (operationalYear == 2014) { samplesAgricultureLimit = 750; }
    else if (operationalYear == 2015) { samplesAgricultureLimit = 800; }
    else if (operationalYear == 2016) { samplesAgricultureLimit = 800; }
    else if (operationalYear == 2017) { samplesAgricultureLimit = 800; }
    else if (operationalYear == 2018) { samplesAgricultureLimit = 850; }
    else if (operationalYear == 2019) { samplesAgricultureLimit = 900; }
    else if (operationalYear == 2020) { samplesAgricultureLimit = 900; }
    else if (operationalYear >= 2021 && operationalYear <= 2022) { samplesAgricultureLimit = 900; }
    else if (operationalYear >= 2023) { samplesAgricultureLimit = 900; }

    // Dynamic adjustment of complementary Strict Agric (Ref 19)
    ...
    var classStrictAgricSamples = primaryStableSamplesFC.filter(ee.Filter.eq('reference', 19)); // Bug fixed: sampling Ref 19 instead of Ref 33
    ...
    // Merge stable vectors (Excluding Non-Veg Ref 25 as requested in Reg 6 script)
    ...
    var compStrictAgricPoints = shuffleCollection(complementarySamplesFC.filter(ee.Filter.eq('reference', 19)), randomSeed).limit(compStrictAgricLimit); // Replaces broken manual patch logic

 ...
 *   - pontos_trained_stable_cleaned_v{samplesVersion}_reg8_trained
 *   - pontos_trained_stable91011_cleaned_v{samplesVersion}_reg8_trained
 *
 * OUTPUTS
 *   - PANT_col4_reg8_v{outputVersion} (one band per year, 2017-2025)
 *
 * NOTES
 ...
 *   - Same three extra classes as region 7 (11, 19 and 29) in both pools.
 *
 * PIPELINE
...
// Define maximum sample allocations for stable training databases (Region 8 - Preserving Legacy Balance)
var samplesForestLimit       = 800;
var samplesSavannaLimit      = 600;
var samplesGrasslandLimit    = 500;
var samplesAgricultureLimitBase = 1000;
...
// Define sample thresholds for complementary datasets (Region 8 - Preserving Legacy Balance)
var compCaveLimit      = 1;
var compSavannaLimit   = 250;
var compGrasslandLimit = 200;
var compForestLimit    = 400;
var compAgroLimit      = 150;
var compNonVegLimit    = 50;
var compWaterLimit     = 1;
var compFloodedLimit   = 50;   // Ref 11 complementar
var compRockyLimit     = 50;   // Ref 29 complementar
...
var assetCollection10 = ee.Image('projects/mapbiomas-public/assets/brazil/lulc/collection10_1/mapbiomas_brazil_collection10_1_coverage_v1');
                          // .mask(pantanalBiomeMask); // left disabled, as in the original
...
//       bands ranked highest for region 8 in the feature-importance run (p01/p05).
...
for (var yearIndex = 0; yearIndex < historicalYearsArray.length; yearIndex++) {
    ...
    // Dynamic adjustment of training allocation parameters (Region 8 progression - Preserving Legacy Balance)
    // The original Landsat ladder used else-if in ascending order, so the later
    // thresholds could never be reached. Rewritten here as sequential ifs.
    // Effective values: 900 for 2017-2019, 1000 for 2020-2022, 1100 from 2023.
    var currentSamplesAgricultureLimit = samplesAgricultureLimitBase;
    if (operationalYear <= 2000) { currentSamplesAgricultureLimit = 700;  }
    if (operationalYear >= 2001) { currentSamplesAgricultureLimit = 800;  }
    if (operationalYear >= 2010) { currentSamplesAgricultureLimit = 900;  }
    if (operationalYear >= 2020) { currentSamplesAgricultureLimit = 1000; }
    if (operationalYear >= 2023) { currentSamplesAgricultureLimit = 1100; }

    // Complementary class 19 quota, with thresholds specific to regions 7 and 8.
    ...
    if (flagCollectionSamples) {
        Map.addLayer(finalRegionalImage, {
            bands: ['swir1_median_dry', 'swir2_median', 'red_median_wet'],
            gain: [0.08, 0.06, 0.2],
            gamma: 0.85
    ...
    // Split the pool by class. (Region 8 source mapping)
    // Standard classes.
    ...
    // Extra classes, specific to regions 7 and 8.
    ...
    // Shuffle, then cap each class at its regional quota. (Region 8 constraints)
    ...
    // Isolate, shuffle, and parameterize regional complementary vector maps (Region 8 constraints)
    ...
    var compNonVegPoints      = shuffleCollection(samplesSentinel.filter(ee.Filter.eq('reference', 25)),randomSeed).limit(compNonVegLimit);
    ...
    var compRockyPoints       = shuffleCollection(samplesLandsat.filter(ee.Filter.eq('reference', 29)), randomSeed).limit(compRockyLimit);
    ...
    // Merge the complementary pools. (todos sempre mesclados)

 ...
 *   - pontos_trained_stable_cleaned_v{samplesVersion}_reg3_trained
 *   - pontos_trained_stable91011_cleaned_v{samplesVersion}_reg3_trained
 *
 * OUTPUTS
 *   - PANT_col4_reg3_v{outputVersion} (one band per year, 2017-2025)
 *
 * NOTES
...
// Define maximum sample allocations for stable training databases (Region 3 - Preserving Legacy Balance)
...
// Define sample thresholds for complementary datasets (Region 3 - Preserving Legacy Balance)
var compCaveLimit      = 1;
var compSavannaLimit   = 2000;
var compGrasslandLimit = 100;
var compForestLimit    = 150;
...
//       bands ranked highest for region 3 in the feature-importance run (p01/p05).
...
for (var yearIndex = 0; yearIndex < historicalYearsArray.length; yearIndex++) {
    ...
    // Base: 1000; single step-up for >= 1990 (all Sentinel years satisfy this condition)
    var currentSamplesAgricultureLimit = 1200;

    if (flagCollectionSamples) {
        Map.addLayer(finalRegionalImage, {
            bands: ['swir1_median', 'red_edge_1_median', 'red_median'],
            gain: [0.08, 0.06, 0.2],
            gamma: 0.85
    ...
    // Split the pool by class. (Region 3 source mapping)
    ...
    // Shuffle, then cap each class at its regional quota. (Region 3 constraints)
    ...
    // Calculate maximum threshold for restrictive agriculture layer (Region 3 constraints)
    // Sequential ifs rather than else-if, so later thresholds override earlier ones.
    var compStrictAgricLimit = 200;

    // Isolate, shuffle, and parameterize regional complementary vector maps (Region 3 constraints)
    // Region 3 uses compWaterLimit = 50, against 1 in regions 0 and 1.
    ...
    // Region 3: class 19 joins the pool through the year-gated branch only.

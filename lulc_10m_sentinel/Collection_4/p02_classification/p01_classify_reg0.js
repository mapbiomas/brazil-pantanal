 ...
 *   
 *   Region 0 covers the whole biome and sits under every other region during
 *   the merge in p02.
 *
 * INPUTS
 ...
 *   - pontos_trained_stable_cleaned_v{samplesVersion}_reg0_trained
 *   - pontos_trained_stable91011_cleaned_v{samplesVersion}_reg0_trained
 *
 * OUTPUTS
 *   - PANT_col4_reg0_v{outputVersion} (one band per year, 2017-2025)
 *
 * NOTES
...
// Full series. Set flagCollectionSamples to run only targetedYearsList instead.
...
// Buffered region boundary that this script classifies.
...
for (var yearIndex = 0; yearIndex < historicalYearsArray.length; yearIndex++) {
    ...
    var currentSamplesAgricultureLimit = 2500;
    if (operationalYear >= 2018) { currentSamplesAgricultureLimit = 2600; }
    if (operationalYear >= 2019) { currentSamplesAgricultureLimit = 2700; }
    if (operationalYear >= 2020) { currentSamplesAgricultureLimit = 2800; }
    if (operationalYear >= 2021) { currentSamplesAgricultureLimit = 2900; }
    if (operationalYear >= 2023) { currentSamplesAgricultureLimit = 3000; }

    if (flagCollectionSamples) {
        Map.addLayer(finalRegionalImage, {
            bands: ['swir1_median', 'nir_median_wet', 'red_median'],
            gain: [0.08, 0.06, 0.2],
            gamma: 0.85
    ...
    classAgroSamples = shuffleCollection(classAgroSamples, 2).limit(currentSamplesAgricultureLimit);
    ...
    // Complementary pools, capped the same way.
    ...
    var compStrictAgricPoints = samplesLandsat.filter(ee.Filter.eq('reference', 19));

    // Irrigated agriculture is rare and grows late in the series.
